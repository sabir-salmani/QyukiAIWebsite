...
// Make.com webhook URL (note: best practice is proxy via your server to avoid CORS)
const WEBHOOK_URL = "https://hook.eu1.make.com/ar3wh325vtob8l83p5kdu34o734nemvi";
const MAKE_API_KEY = "9167166088xyz";
...
const Footer: React.FC = () => {
  ...
  const footerLinks = {
    ...
    Legal: [
      { name: "Privacy Policy", href: "/privacy" },
      // add real routes if you have these pages
  ...
  const renderFooterLink = (href: string, label: string) => {
    const isInternal = href.startsWith("/");      // includes "/#pricing"
    const isAnchor = href.startsWith("#");        // on-page only (not used now)
  ...
  return (
    <footer className="bg-black text-white">
      ...
      {/* Bottom Bar */}
      <div className="border-t border-gray-800">
        <div className="max-w-7xl mx-auto px-4 sm:px-2 lg:px-8 py-1">
          <div className="flex flex-col md:flex-row items-center justify-between">
            <p className="text-gray-400 text-sm">
              © 2025 Qyuki AI. All rights reserved.
            </p>
            <div className="flex items-center space-x-6 mt-4 md:mt-0">
              ...
              {/* add if you have pages */}
              {/* <Link to="/terms" className="text-gray-400 hover:text-white text-sm transition-colors duration-200">Terms of Service</Link>
              <Link to="/cookies" className="text-gray-400 hover:text-white text-sm transition-colors duration-200">Cookie Settings</Link> */}
            </div>
          </div>
        </div>
      </div>
